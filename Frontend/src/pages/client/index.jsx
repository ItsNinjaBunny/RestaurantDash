...
const Client = () => {
 ...
 function AddToCart(obj){
    ...
    console.log(user.cart.items);
    if (index !== -1) {
      user.cart.items[index].qty=user.cart.items[index].qty+1;
  ...
  const updateTotal=()=>{
    ...
    var x =0;
      for (let index = 0; index < user.cart.items.length; index++) {
        var element = user.cart.items[index].price * user.cart.items[index].qty;
        x = x+element ;
  ...
  return (
    ...
    <>
      ...
      <Content className={active==='home'||'orders'||'cart'||'coupons' ? animation : ''} >
        ...
        {active==="cart" && 
        <>
         <Cart delFromCart={delFromCart} EditItemQty={EditItemQty} cart={user.cart}></Cart>
